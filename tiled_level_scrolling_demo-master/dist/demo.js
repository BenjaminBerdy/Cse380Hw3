...
game.getResourceManager().loadScene(DESERT_SCENE_PATH, game.getSceneGraph(), game.getRenderingSystem(), function () {
    ...
    for (var i = 0; i < 50; i++) {
        for (var j = 0; j < 50; j++) {
            var typeTile = game.getResourceManager().getAnimatedSpriteType("DESERT");
            var tile = new AnimatedSprite_1.AnimatedSprite(typeTile, "DESERT", "map", game);
    ...
    for (var _i = 0; _i < 50; _i++) {
        var _type = game.getResourceManager().getAnimatedSpriteType("DEVILS_FLOWER_MANTIS");
        var randomSprite = new AnimatedSprite_1.AnimatedSprite(_type, "WALKING", "devil", game);
    ...
    for (var _i2 = 0; _i2 < 50; _i2++) {
        var _type2 = game.getResourceManager().getAnimatedSpriteType("FLOWER_MANTIS");
        var _randomSprite = new AnimatedSprite_1.AnimatedSprite(_type2, "WALK", "flower", game);
        ...
        _randomSprite.getLocalPosition().set(_randomSprite.getPosition().getX(), _randomSprite.getPosition().getY(), 0, 1);
        game.getSceneGraph().addAnimatedSprite(_randomSprite);
    }
    var type = game.getResourceManager().getAnimatedSpriteType("BEE_LARVAE2");
    var weedle = new AnimatedSprite_1.AnimatedSprite(type, "WALKING", "weedle", game);
    ...
    var spritesInViewportText = new TextRenderer_1.TextToRender("Sprites in Viewport", "", 20, 90, function () {
        var k = 0;
        sceneGraph.scope();
        for (var _i3 = 0; _i3 < sceneGraph.getVisibleSet().length; _i3++) {
            if (sceneGraph.getVisibleSet()[_i3].getTypeString() != "map") {
                k++;
            }
        }
    ...
    var weedleRotationText = new TextRenderer_1.TextToRender("weedleRotation", "", 20, 130, function () {
...
},{"../wolfie2d/Game":2,"../wolfie2d/rendering/TextRenderer":9,"../wolfie2d/scene/sprite/AnimatedSprite":20}],2:[function(require,module,exports){
"use strict";
...
var Game = function (_GameLoopTemplate_1$G) {
    ...
    function Game(gameCanvasId, textCanvasId) {
        ...
        _this.gameCanvas = document.getElementById(gameCanvasId);
    ...
    _createClass(Game, [{
        ...
        value: function update(delta) {
            if (this.gamePhysics.getDeadCount() >= 50) {
                var youWinText = new TextRenderer_1.TextToRender("YOU WIN", "", 100, 500, function () {
...
},{"../wolfie2d/rendering/TextRenderer":9,"./files/ResourceManager":3,"./loop/GameLoopTemplate":4,"./physics/GamePhysics":8,"./rendering/WebGLGameRenderingSystem":11,"./scene/SceneGraph":17,"./scene/Viewport":19,"./ui/UIController":24}],3:[function(require,module,exports){
"use strict";
...
},{"../rendering/WebGLGameTexture":14,"../scene/sprite/AnimatedSpriteType":21,"../scene/tiles/TileSet":22,"../scene/tiles/TiledLayer":23}],4:[function(require,module,exports){
"use strict";
...
var GamePhysics = function () {
    ...
    _createClass(GamePhysics, [{
        ...
        value: function update(sceneGraph) {
            ...
            var animatedSprites = sceneGraph.getAnimatedSprites();

            var _loop = function _loop(i) {
                if (animatedSprites[i].getTypeString() != "map") {
                    if (sceneGraph.getMainCharacterX() + sceneGraph.getMainCharacterWidth() > animatedSprites[i].getPosition().getX() && sceneGraph.getMainCharacterX() + sceneGraph.getMainCharacterWidth() < animatedSprites[i].getPosition().getX() + animatedSprites[i].getSpriteType().getSpriteWidth() && sceneGraph.getMainCharacterY() + sceneGraph.getMainCharacterHeight() > animatedSprites[i].getPosition().getY() && sceneGraph.getMainCharacterY() + sceneGraph.getMainCharacterHeight() < animatedSprites[i].getPosition().getY() + animatedSprites[i].getSpriteType().getSpriteHeight() || sceneGraph.getMainCharacterX() > animatedSprites[i].getPosition().getX() && sceneGraph.getMainCharacterX() < animatedSprites[i].getPosition().getX() + animatedSprites[i].getSpriteType().getSpriteWidth() && sceneGraph.getMainCharacterY() > animatedSprites[i].getPosition().getY() && sceneGraph.getMainCharacterY() < animatedSprites[i].getPosition().getY() + animatedSprites[i].getSpriteType().getSpriteHeight()) {
                        if (animatedSprites[i].getStatus() == "alive") {
                            if (animatedSprites[i].getTypeString() == "devil") {
                                ...
                                setTimeout(function () {
                                    animatedSprites[i].setState("DEAD");
                                }, 500);
...
var WebGLGameSpriteRenderer = function (_WebGLGameRenderingCo) {
    ...
    _createClass(WebGLGameSpriteRenderer, [{
        ...
        value: function renderAnimatedSprite(webGL, viewport, sprite) {
            ...
            var spriteXInPixels = sprite.getLocalPosition().getX() + spriteWidth / 2;
            var spriteYInPixels = sprite.getLocalPosition().getY() + spriteHeight / 2;
            ...
            this.meshRotate.setThetaZ(sprite.getRotation().getThetaZ());
            ...
            this.meshScale.set(sprite.getScale().getX() * scaleX, sprite.getScale().getY() * scaleY, 0.0, 0.0); //1.0, 1.0);
...
var BugAI = function () {
    function BugAI(sprite, game, type) {
    ...
    _createClass(BugAI, [{
            ...
            if (this.sprite.getTypeString() != "map") {
                if (this.sprite.getStatus() != "dead") {
                    if (this.type === "weedle") {
                        ...
                        if (Math.sqrt(Math.pow((scene.getMainCharacterX() + (scene.getMainCharacterX() + scene.getMainCharacterWidth())) / 2 - (this.sprite.getPosition().getX() + (this.sprite.getPosition().getX() + this.sprite.getSpriteType().getSpriteWidth())) / 2, 2) + Math.pow((scene.getMainCharacterY() + (scene.getMainCharacterY() + scene.getMainCharacterHeight())) / 2 - (this.sprite.getPosition().getY() + (this.sprite.getPosition().getY() + this.sprite.getSpriteType().getSpriteHeight())) / 2, 2)) < 150) {
                            this.run(scene);
                        } else {
        ...
        key: "walk",
        value: function walk(scene) {
            var rotation = Math.floor(this.sprite.getRotation().getThetaZ() * 10);
            if (rotation == 0) {
        ...
        key: "checkBounds",
        value: function checkBounds(scene) {
            var world = scene.getTiledLayers();
            var worldWidth = world[0].getColumns() * world[0].getTileSet().getTileWidth();
            var worldHeight = world[0].getRows() * world[0].getTileSet().getTileHeight();
        ...
        value: function run(scene) {
            var spriteX = this.sprite.getPosition().getX();
            var spriteY = this.sprite.getPosition().getY();
            var weedleX = scene.getMainCharacterX();
            var weedleY = scene.getMainCharacterY();
        ...
        key: "moveWeedle",
        value: function moveWeedle(scene) {
            var mouseX = this.game.getUiController().getMouseX();
            var mouseY = this.game.getUiController().getMouseY();
            if (mouseX != undefined && mouseY != undefined) {
                var angle = -Math.atan2(this.sprite.getLocalPosition().getY() + this.sprite.getSpriteType().getSpriteHeight() / 2 - mouseY, this.sprite.getLocalPosition().getX() + this.sprite.getSpriteType().getSpriteWidth() / 2 - mouseX) - 3 * Math.PI / 2;
                this.sprite.getRotation().setThetaZ(angle);
                if (this.sprite.getRotation().getThetaZ() <= -6.5 && this.sprite.getRotation().getThetaZ() > -7 || this.sprite.getRotation().getThetaZ() >= -6 && this.sprite.getRotation().getThetaZ() < -5 || this.sprite.getRotation().getThetaZ() >= -2.8 && this.sprite.getRotation().getThetaZ() < -2 || this.sprite.getRotation().getThetaZ() <= 3.5 && this.sprite.getRotation().getThetaZ() > -4) {
                    this.sprite.getScale().set(1.1, .8, 0, 1);
                } else if (this.sprite.getRotation().getThetaZ() <= -4 && this.sprite.getRotation().getThetaZ() >= -5 || this.sprite.getRotation().getThetaZ() >= -2 || this.sprite.getRotation().getThetaZ() <= -7) {
        ...
        key: "weedleRun",
        value: function weedleRun(scene) {
            ...
            if (this.sprite.getStatus() != "flee") {
                ...
                setTimeout(function () {
                    scene.getMainCharacter().setState("WALKING");scene.getMainCharacter().setStatus("alive");
                }, 2000);
            }
        }
    }]);
...
var SceneGraph = function () {
    function SceneGraph() {
        ...
        this.mainCharacterX = 200;
        this.mainCharacterY = 200;
        this.mainCharacterWidth = 50;
        this.mainCharacterHeight = 50;
    ...
    _createClass(SceneGraph, [{
        ...
        key: "setMainCharacterX",
        value: function setMainCharacterX(mainCharacterX) {
            this.mainCharacterX = mainCharacterX;
        }
    }, {
        key: "setMainCharacterY",
        value: function setMainCharacterY(mainCharacterY) {
            this.mainCharacterY = mainCharacterY;
        }
    }, {
        key: "getMainCharacterX",
        value: function getMainCharacterX() {
            return this.mainCharacterX;
        }
    }, {
        key: "getMainCharacterY",
        value: function getMainCharacterY() {
            return this.mainCharacterY;
        }
    }, {
        key: "setMainCharacterWidth",
        value: function setMainCharacterWidth(mainCharacterWidth) {
            this.mainCharacterWidth = mainCharacterWidth;
        }
    }, {
        key: "setMainCharacterHeight",
        value: function setMainCharacterHeight(mainCharacterHeight) {
            this.mainCharacterHeight = mainCharacterHeight;
        }
    }, {
        key: "getMainCharacterWidth",
        value: function getMainCharacterWidth() {
            return this.mainCharacterWidth;
        }
    }, {
        key: "getMainCharacterHeight",
        value: function getMainCharacterHeight() {
        ...
        value: function setMainCharacter(mainCharacter) {
        ...
        value: function update(delta) {
            ...
            for (var i = 0; i < this.getVisibleSet().length; i++) {
                this.getVisibleSet()[i].getLocalPosition().set(this.getVisibleSet()[i].getPosition().getX() - this.getViewport().getX(), this.getVisibleSet()[i].getPosition().getY() - this.getViewport().getY(), 0, 1);
            }
        }
    }, {
        key: "scope",
        value: function scope() {
            ...
            try {
                for (var _iterator3 = this.animatedSprites[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                    ...
                    if (sprite.getPosition().getX() + sprite.getSpriteType().getSpriteWidth() > this.viewport.getX() && sprite.getPosition().getX() < this.viewport.getX() + this.viewport.getWidth() && sprite.getPosition().getY() + sprite.getSpriteType().getSpriteHeight() > this.viewport.getY() && sprite.getPosition().getY() < this.viewport.getY() + this.viewport.getHeight()) {
...
},{"../math/Vector3":7}],19:[function(require,module,exports){
"use strict";
...
var BugAI_1 = require("../BugAI");

var AnimatedSprite = function (_SceneObject_1$SceneO) {
    _inherits(AnimatedSprite, _SceneObject_1$SceneO);

    function AnimatedSprite(initSpriteType, initState, initSpriteTypeString, game) {
        ...
        _this.typeString = initSpriteTypeString;
        _this.BugAI = new BugAI_1.BugAI(_this, game, _this.typeString);
        if (_this.typeString == "map") {
            _this.status = "dead";
        } else {
            _this.status = "alive";
        }
    ...
    _createClass(AnimatedSprite, [{
        ...
        key: "update",
        value: function update(delta, scene) {
            this.frameCounter++;
...
},{"../BugAI":16,"../SceneObject":18}],21:[function(require,module,exports){
"use strict";
...
var UIController = function () {
    function UIController(canvasId, initScene) {
        ...
        this.keyPressHandler = function (event) {
            if (event.keyCode === 119) {
                _this.changeView(0, -10);
            } else if (event.keyCode === 97) {
                _this.changeView(-10, 0);
    ...
    _createClass(UIController, [{
        ...
        key: "changeView",
        value: function changeView(x, y) {
            var world = this.scene.getTiledLayers();
            var worldWidth = world[0].getColumns() * world[0].getTileSet().getTileWidth();
            var worldHeight = world[0].getRows() * world[0].getTileSet().getTileHeight();
            if (this.scene.getViewport().getX() + x > worldWidth - this.scene.getViewport().getWidth() || this.scene.getViewport().getX() + x < 0 || this.scene.getViewport().getY() + y > worldHeight - this.scene.getViewport().getHeight() || this.scene.getViewport().getY() + y < 0) {
                ;
            } else {
